...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		bitVoteLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		bitVoteLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		bitVoteLog.Infof("Creating block database in memory.")
	...
	bitVoteLog.Infof("Loading block database from '%s'", dbPath)
	...
	bitVoteLog.Info("Block database loaded")
	return db, nil
}
...
// bitVoteMain is the real main function for bitVote.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func bitVoteMain(serverChan chan<- *server) error {
	...
	defer bitVoteLog.Info("Shutdown complete")

	// Show version at startup.
	bitVoteLog.Infof("Version %s", version())

	// Enable http profiling server if requested.
	if cfg.Profile != "" {
		go func() {
			listenAddr := net.JoinHostPort("", cfg.Profile)
			bitVoteLog.Infof("Profile server listening on %s", listenAddr)
			profileRedirect := http.RedirectHandler("/debug/pprof",
				http.StatusSeeOther)
			http.Handle("/", profileRedirect)
			bitVoteLog.Errorf("%v", http.ListenAndServe(listenAddr, nil))
		}()
	}
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			bitVoteLog.Errorf("Unable to create cpu profile: %v", err)
			return err
		}
	...
	if cfg.MemoryProfile != "" {
		...
		if err != nil {
			bitVoteLog.Errorf("Unable to create memory profile: %v", err)
			return err
		}
		defer f.Close()
	...
	// Perform upgrades to bitVote as new versions require it.
	if err := doUpgrades(); err != nil {
		bitVoteLog.Errorf("%v", err)
		return err
	}
	...
	defer func() {
		...
		bitVoteLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		bitVoteLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		bitVoteLog.Infof("Gracefully shutting down the server...")
		server.Stop()
...
// winServiceMain is only invoked on Windows.  It detects when bitVote is running
...
func main() {
	...
	if err := bitVoteMain(nil); err != nil {
		os.Exit(1)
	}
}
