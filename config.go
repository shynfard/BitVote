...
const (
	defaultConfigFilename        = "bitVote.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "bitVote.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-bitVote.conf"
...
var (
	defaultHomeDir     = btcutil.AppDataDir("bitVote", false)
...
// config defines the configuration options for bitVote.
//
...
type config struct {
	...
	AgentBlacklist       []string      `long:"agentblacklist" description:"A comma separated list of user-agent substrings which will cause bitVote to reject any peers whose user-agent contains any of the blacklisted substrings."`
	AgentWhitelist       []string      `long:"agentwhitelist" description:"A comma separated list of user-agent substrings which will cause bitVote to require all peers' user-agents to contain one of the whitelisted substrings. The blacklist is applied before the whitelist, and an empty whitelist will allow all agents that do not fail the blacklist."`
...
//
// The above results in bitVote functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		bitVoteLog.Infof("RPC service is disabled")
	}
	...
	if configFileError != nil {
		bitVoteLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-bitVote.conf to the given destination path,
...
// bitVoteDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func bitVoteDial(addr net.Addr) (net.Conn, error) {
...
// bitVoteLookup resolves the IP of the given host using the correct DNS lookup
...
func bitVoteLookup(host string) ([]net.IP, error) {
