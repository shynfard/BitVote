...
var helpDescsEnUS = map[string]string{
	...
	"sendrawtransaction-feesetting":   "Whether or not to allow insanely high fees in bitcoind < v0.19.0 or the max fee rate for bitcoind v0.19.0 and later (bitVote does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown bitVote.",
	"stop--result0":  "The string 'bitVote stopping.'",
	...
	// NotifySpentCmd help.
	"notifyspent--synopsis": "Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this bitVote instance) and when such a transaction first appears in a newly-attached block.",
