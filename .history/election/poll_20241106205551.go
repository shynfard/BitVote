...
func VerifyPoll(poll *Poll) bool {
	// Check money
	money := blockchain.GetMoneyByPublicKey(poll.Creator.GetPublicKey().Bytes())
	if poll.fee < money {
		fmt.Println("Not enough blocked money for the user")
		return false
	}
	// verify signature
	expectedHash := poll.Hash()
	pubKey := ecdsa.PublicKey{}
	pubKey.SetBytes(poll.creatorPublicKey)
	if ok, err := pubKey.Verify(poll.signature, expectedHash, sha256.New()); !ok || err != nil {
