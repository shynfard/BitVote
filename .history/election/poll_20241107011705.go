...
type PollPublic struct {
	...
	HomomorphicPublicKeyN        *big.Int `json:"homomorphicPublicKeyN"`
	HomomorphicPublicKeyG        *big.Int `json:"homomorphicPublicKeyG"`
	HomomorphicPublicKeyNSquared *big.Int `json:"homomorphicPublicKeyNSquared"`
}
...
func (p *Poll) GetPoll() []byte {
	pollPublic := map[string]interface{}{
		...
		"question":                     p.question,
		"homomorphicPublicKeyG":        p.homomorphicPublicKey.G,
		"homomorphicPublicKeyN":        p.homomorphicPublicKey.N,
		"homomorphicPublicKeyNSquared": p.homomorphicPublicKey.NSquared,
		"options":                      p.options,
		"duration":                     p.duration,
...
func LoadPoll(data []byte) (*Poll, error) {
	...
	poll := Poll{
		...
		signature:        p.Signature,
		homomorphicPublicKey: &paillier.PublicKey{
			N:        p.HomomorphicPublicKeyN,
			G:        p.HomomorphicPublicKeyG,
			NSquared: p.HomomorphicPublicKeyNSquared,
		},
	}
	return &poll, nil
}
...
func (p *Poll) CountVotes() []big.Int {
	...
	for _, vote := range p.votes {
		for _, encVote := range vote.encryptedVote {

			encryptedSum.Mul(encryptedSum, vote.encryptedVote)
			encryptedSum.Mod(encryptedSum, p.homomorphicPublicKey.N)
	}
	return encryptedSum
