...
func CreatePoll(creatorPrivateKey []byte, question []byte, options [][]byte, duration int, participants [][]byte) *Poll {
	...
	ok, err := priv.Public().Verify(signature, hash, sha256.New())
	fmt.Println("ok: ", ok)

	newPublicKey := ecdsa.PublicKey{}
	newPublicKey.SetBytes(priv.Public().Bytes())
	ok, err = newPublicKey.Verify(signature, hash, sha256.New())
	fmt.Println("ok: ", ok)
	return p
}
...
func VerifyPoll(poll *Poll) bool {
	...
	ok, err := pubKey.Verify(poll.signature, poll.GetPoll(), sha256.New())
	fmt.Println(ok)
	fmt.Println(err)
	if !ok || err != nil {
		fmt.Println("Poll signature verification failed")
		fmt.Println(poll.signature)
		return false
	}
	return true
}
