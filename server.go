...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			bitVoteLookup, func(addrs []*wire.NetAddressV2) {
...
// newServer returns a new bitVote server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, bitVoteLookup)
	...
	if cfg.Prune != 0 {
		bitVoteLog.Infof("Prune set to %d MiB", cfg.Prune)
	}
