...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-bitVote.conf")

	// Setup a temporary directory
	tmpDir, err := os.MkdirTemp("", "bitVote")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of bitVote binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-bitVote.conf")
