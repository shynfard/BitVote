...
// oldBitVoteHomeDir returns the OS specific home directory bitVote used prior to
...
func oldBitVoteHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "bitVote")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".bitVote")
	}
...
// location prior to bitVote version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to bitVote
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldBitVoteHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bitVote.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bitVote_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bitVote_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to bitVote
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		bitVoteLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old bitVote.conf into new location if needed.
		...
		if ohpEmpty {
			...
			bitVoteLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to bitVote as new versions require it.
