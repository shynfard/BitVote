...
const (
	// svcName is the name of bitVote service.
	svcName = "bitVotesvc"
	...
	svcDisplayName = "BitVote Service"
...
// logServiceStartOfDay logs information about bitVote when the main server has
...
// bitVoteService houses the main service handler which handles all service
// updates and launching bitVoteMain.
type bitVoteService struct{}
...
// long-running bitVoteMain (which is the real meat of bitVote), handles service
// change requests, and notifies the service control manager of changes.
func (s *bitVoteService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start bitVoteMain in a separate goroutine so the service can start
...
// installService attempts to install the bitVote service.  Typically this should
...
// removeService attempts to uninstall the bitVote service.  Typically this should
...
// startService attempts to start the bitVote service.
...
func serviceMain() (bool, error) {
	...
	// needed to run bitVote on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &bitVoteService{})
	if err != nil {
